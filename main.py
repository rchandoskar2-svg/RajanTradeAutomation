...
# Version: 1.2 – NSE+Fyers wired, WebSocket REALTIME ON
# + Fyers T1 Auth redirect handler
# ==========================================================
...
# Fyers REST client (token पुढे update करू शकतो)
...
def fetch_market_breadth():
    """
    Primary: NSE allIndices → pick NIFTY 50 breadth.
    Fallback: adv=dec=0 (no bias).
    """
    ...
    log("fetch_market_breadth: NSE failed, fallback adv=dec=0.")
    return 0, 0
...
def fetch_sector_performance():
    """
    NSE allIndices → percentChange per index.
    ...
    if data and "data" in data:
        try:
            ...
            log(f"fetch_sector_performance: got {len(sectors)} sectors.")
        except Exception as e:
...
def fetch_constituents_for_index(index_name):
    ...
    if data and "data" in data:
        try:
            ...
            log(f"parse constituents error {index_name}: {e}")
    return out
...
def fetch_fno_universe():
    """
    For each sector index, get constituents from NSE
    then convert to Fyers symbols.
    """
    universe = []
    seen = set()
    for idx_name in SECTOR_INDEX_NAMES:
        ...
        for row in cons:
            ...
            universe.append(row)
    log(f"fetch_fno_universe: total approx symbols={len(universe)}")
    return universe
...
def fyers_quotes(symbols):
    """Fetch LTP, %change, volume for given symbols."""
    if not symbols:
        return {}
    try:
...
def fyers_5min_history(symbol, start: datetime, end: datetime):
    ...
    try:
        ...
        log(f"fyers_5min_history error {symbol}: {e}")
        return []
...
def prepare_day_if_needed():
    ...
    # फक्त 09:26–09:30 मध्ये selection करायची
    now = now_ist()
...
def evaluate_candle(symbol: str):
    ...
    target_price = (
        entry_price + rr * risk_per_share
        if direction == "BUY"
        else entry_price - rr * risk_per_share
    )

    qty = 1
    per_trade_risk_en = (
        str(settings.get("ENABLE_PER_TRADE_RISK", "TRUE")).upper() == "TRUE"
    )
...
def check_exits():
    with engine_lock:
        symbols_with_trades = [
            s for s, st in engine_state["symbols"].items() if st.get("active_trade")
        ]
        settings = engine_state.get("settings", {})
    ...
    auto_sq_dt = now.replace(
        hour=auto_h, minute=auto_m, second=0, microsecond=0
    )
...
# ----------------------------------------------------------
# FYERS AUTH ROUTES (NEW)
# ----------------------------------------------------------
@app.route("/fyers-login", methods=["GET"])
def fyers_login():
    """
    Browser मध्ये हे hit केले तर Fyers auth URL परत करतो.
    """
    if not FYERS_CLIENT_ID or not FYERS_REDIRECT_URI:
        return jsonify({"ok": False, "error": "Missing CLIENT_ID or REDIRECT_URI"}), 400
...
def fyers_redirect():
    """
    Fyers console मधील redirect_uri म्हणून हाच URL दिलेला आहे.
    Redirect झाल्यावर auth_code capture करतो.
    404 येण्याची समस्या आता संपली.
    """
    code = request.args.get("auth_code")
    status = request.args.get("s") or request.args.get("status")
    message = request.args.get("message")

    print("=== FYERS REDIRECT RECEIVED ===")
    print("Status:", status)
    print("Auth Code:", code)
    print("Message:", message)

    if not code:
        return "Auth Code not received", 400

    # इथे फक्त auth_code दाखवतो. Token generate करणे नंतर करू.
    html = f"""
    <html>
    <body>
      <h3>Fyers Auth Code Received ✔</h3>
      <p><b>Status:</b> {status}</p>
      <p><b>Auth Code (copy & save safely):</b></p>
      <textarea rows="4" cols="80">{code}</textarea>
      <p>हा code कुणालाही share करू नकोस. Render env मधील FYERS_ACCESS_TOKEN तयार करताना याचा वापर कर.</p>
    </body>
    </html>
    """
    return html
